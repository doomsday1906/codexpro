...
console.log("# RepoConnect M009 TASK-007R4 Comprehensive Falsifiers");
...
async function runFalsifiers() {
  ...
  httpProcess.stderr.on("data", (chunk) => {
    ...
    if (text.includes("HTTP MCP listening")) httpReady = true;
  });
  for (let i = 0; i < 50; i++) {
    if (httpReady) break;
    await sleep(100);
  }
  assert.ok(httpReady, "HTTP server must be listening within 5 seconds");

  const transport = new StreamableHTTPClientTransport(new URL(`http://127.0.0.1:${port}/mcp`));
  const client = new Client({ name: "r4-test-client", version: "1.0.0" }, { capabilities: {} });
  ...
  for (const pid of descendants) {
    ...
    assert.ok(dead, `Descendant PID ${pid} must be dead after HTTP SIGTERM`);
    console.log(`    PID ${pid}: DEAD confirmed`);
  }

  assert.ok(isPidAlive(decoyPid), `Decoy PID ${decoyPid} must survive HTTP shutdown because it is not owned`);
  ...
  // Direct manager lifecycle & race proof
  // =========================================================================
  ...
  // =========================================================================
  // Finding 5: MEDIUM-SHUTDOWN-RACE-PROOF-001 Strengthened Real-HTTP Controlled-Shutdown Race
  // =========================================================================
  console.log("\n--- [5] Testing MEDIUM-SHUTDOWN-RACE-PROOF-001 (Real-HTTP Controlled-Shutdown Race) ---");
  ...
  const raceHttpClient = new Client({ name: "r4-http-race-client", version: "1.0.0" }, { capabilities: {} });
  ...
  // 5.2: Send SIGTERM to HTTP server and deliberately issue multiple late starts around shutdown boundary
  console.log("  Sending SIGTERM to HTTP server and issuing rapid concurrent late start requests...");
  ...
  for (let i = 0; i < 5; i++) {
    try {
      const res = await raceHttpClient.callTool({
        name: "start_verification",
        arguments: {
          workspace_id: realWsId,
          runner: "package_script",
      ...
      const isToolError = Boolean(res.isError);
      const text = res.content?.map(c => c.text).join(" ") || "";
      assert.ok(isToolError, `Late call #${i} must NOT succeed as a valid job`);
      assert.ok(!res.structuredContent?.jobId, `Late call #${i} must not return a new jobId`);
      assert.match(text, /verification_manager_closing/i, `Late call #${i} must indicate manager closing`);
      lateDispositions.push({ call: i, result: "tool_error_closing", isError: true });
    } catch (err) {
      // Transport closed / connection reset / session not found
      lateDispositions.push({ call: i, result: "transport_closed", error: err.message });
    }
  }
  ...
  console.log("PASS: MEDIUM-SHUTDOWN-RACE-PROOF-001 (Real-HTTP) fully closed and verified.");

  // =========================================================================
  // Finding 6: MEDIUM-SHUTDOWN-RACE-PROOF-001 Real-Stdio Controlled-Shutdown Race
  // =========================================================================
  console.log("\n--- [6] Testing MEDIUM-SHUTDOWN-RACE-PROOF-001 (Real-Stdio Controlled-Shutdown Race) ---");
  ...
  const stdioClient = new Client({ name: "r4-stdio-race-client", version: "1.0.0" }, { capabilities: {} });
  ...
  console.log("  Sending SIGTERM to stdio server and issuing rapid concurrent late start requests...");
  ...
  for (let i = 0; i < 5; i++) {
    try {
      const res = await stdioClient.callTool({
        name: "start_verification",
        arguments: {
          workspace_id: realWsId,
          runner: "package_script",
      ...
      assert.ok(!res.structuredContent?.jobId, `Late stdio call #${i} must not return a new jobId`);
      assert.match(text, /verification_manager_closing/i, `Late stdio call #${i} must indicate manager closing`);
      stdioLateDispositions.push({ call: i, result: "tool_error_closing", isError: true });
    } catch (err) {
      stdioLateDispositions.push({ call: i, result: "transport_closed", error: err.message });
    }
  }
  ...
  console.log("PASS: MEDIUM-SHUTDOWN-RACE-PROOF-001 (Real-Stdio) fully closed and verified.");

  // =========================================================================
  // Finding 7: HIGH-REDACTION-TAIL-CONTEXT-001 Falsifiers
  // Prefix-cut, chunk-boundary, transport/direct-pattern, multiline private keys,
  // and output regressions
  // =========================================================================
  console.log("\n--- [7] Testing HIGH-REDACTION-TAIL-CONTEXT-001 (Redaction Tail Context & Output Bounds) ---");

  // 7.1: Falsifier A — Prefix-cut ordinary credential witness
  console.log("  Testing 7.1: Prefix-cut ordinary credential witness...");
  const fakeSecretVal = "ordinarysecret123";
  const budgetA = 24; // Small budget: under R3 raw buffer, "MY_SUPER_SECRET_TOKEN=" would be discarded and ordinarysecret123 retained!
  ...
  console.log("    Result A stdout:", JSON.stringify(resA.stdout));
  ...
  assert.ok(resA.stdout.includes("REDACTED_SECRET"), "stdout must contain REDACTED_SECRET marker");
  ...
  assert.ok(resA.observedStdoutBytes > 0, "observedStdoutBytes must truthfully reflect physical bytes");
  assert.ok(resA.observedStdoutBytes >= `MY_SUPER_SECRET_TOKEN=${fakeSecretVal}\n`.length, "observedStdoutBytes must reflect unredacted raw length");
  console.log("    PASS 7.1: Prefix-cut ordinary credential withheld, truncated=true, observed bytes truthful");

  // 7.2: Falsifier B — Chunk-boundary credential witness
  console.log("  Testing 7.2: Chunk-boundary credential witness...");
  ...
  console.log("    Result B stdout:", JSON.stringify(resB.stdout));
  ...
  assert.ok(Buffer.byteLength(resB.stdout, "utf8") + Buffer.byteLength(resB.stderr, "utf8") <= budgetB, "Combined bytes must be <= budget");
  console.log("    PASS 7.2: Chunk-boundary split credential redacted safely under retention pressure");

  // 7.3: Falsifier C — Transport/direct-pattern boundary witness
  console.log("  Testing 7.3: Transport/direct-pattern boundary witness...");
  const fakeAuthVal = "myauthtokenXYZ";
  const budgetC = 40;
  const configC = loadConfig(["--root", repoRoot]);
  configC.maxOutputBytes = budgetC;
  const guardC = new PathGuard(configC);
  const mgrC = new VerificationManager(configC, { retainedTailBytes: budgetC });

  const jobC = await mgrC.startVerification(fakeWorkspace, guardC, {
    workspace_id: fakeWorkspace.id,
    runner: "package_script",
    package_manager: "npm",
    script: "verification:fixture",
    args: ["--emit-authorization", fakeAuthVal, "80", "--exit", "0"]
  });
  const resC = await mgrC.waitVerification(jobC.jobId, 5000);

  console.log("    Result C stdout:", JSON.stringify(resC.stdout));
  assert.ok(!resC.stdout.includes(fakeAuthVal), `stdout must NOT contain ${fakeAuthVal}`);
  assert.ok(!resC.stderr.includes(fakeAuthVal), `stderr must NOT contain ${fakeAuthVal}`);
  assert.ok(Buffer.byteLength(resC.stdout, "utf8") + Buffer.byteLength(resC.stderr, "utf8") <= budgetC, "Combined bytes must be <= budget");
  console.log("    PASS 7.3: Authorization-style direct pattern preserved without value leakage");

  // 7.4: Falsifier D — Multiline/private-key-context witness
  console.log("  Testing 7.4: Multiline/private-key-context witness...");
  const budgetD = 50;
  ...
  console.log("    Result D stdout:", JSON.stringify(resD.stdout));
  ...
  assert.ok(Buffer.byteLength(resD.stdout, "utf8") + Buffer.byteLength(resD.stderr, "utf8") <= budgetD, "Combined bytes must be <= budget");

  // Also test unclosed private key (no END marker)
  ...
  console.log("    Result D2 (unclosed) stdout:", JSON.stringify(resD2.stdout));
  assert.ok(!resD2.stdout.includes("fakeunclosedkeymaterialline"), "Unclosed private key body lines must NOT leak");
  console.log("    PASS 7.4: Multiline and unclosed private key body material suppressed and safe");

  // 7.5: Falsifier E — Output regressions (redaction-expansion, multibyte UTF-8, 60k+60k, output ceiling)
  console.log("  Testing 7.5: Output regressions...");

  // E.1: Redaction-expansion
  ...
  console.log("    PASS E.1: Redaction-expansion output strictly bounded");
  ...
  console.log("    PASS E.2: Multibyte UTF-8 trimming preserves codepoint boundaries");
  ...
  console.log("    PASS E.3: Combined stdout+stderr budget strictly enforced");
  ...
  console.log("    PASS E.4: Process terminated on hard output ceiling breach");

  console.log("PASS: HIGH-REDACTION-TAIL-CONTEXT-001 fully closed and verified.");
  ...
  console.log("\n=========================================================================");
  console.log("ALL TASK-007R4 FALSIFIERS PASSED (Groups 1 through 8)");
  console.log("=========================================================================");
}
